...
/**
 * Created by odfly on 2017/1/7.
 */
public class DateUtil {
    // 默认日期格式
    public static final String DATE_DEFAULT_FORMAT = "yyyy-MM-dd";

    public static final String DATE_DEFAULT_FORMAT_A = "yyyy/MM/dd";
     ...
     * @return 比较两日期大小。如果date>otherDate则返回true,否则false
     */
    public static Boolean whenGTYun(Date date, Date otherDate) {
        Boolean flag = false;
        Date dateTmp = getDateTime(date);
        Date otherDateTmp = getDateTime(otherDate);
        if (dateTmp != null && otherDateTmp != null) {
...
//        sunCalenday.setFirstDayOfWeek(Calendar.MONDAY);
     ...
     * @return
     */
    public static List<Date> getCrossPeriod(Date firstStart, Date firstEnd,Date secondStart,Date secondEnd) {
        if (firstStart == null || firstEnd == null || secondEnd == null || secondStart == null) {
            return Lists.newArrayList();
        }
        long firstStartLong = firstStart.getTime();
        long firstEndLong = firstEnd.getTime();
        long secondStartLong = secondStart.getTime();
        long secondEndLong = secondEnd.getTime();

        long startLong = 0;
        long endLong = 0;
        if ((secondStartLong >= firstStartLong && secondStartLong <= firstEndLong) || (secondEndLong >= firstStartLong && secondEndLong <= firstEndLong)) {
            startLong = secondStartLong >= firstStartLong ? secondStartLong : firstStartLong;
            endLong = firstEndLong >= secondEndLong ? secondEndLong : firstEndLong;
        } else {
            return Lists.newArrayList();
        }
        return Lists.newArrayList(getDate(startLong),getDate(endLong));
    }
    ...
    public static boolean isDesignMonth(Date date,int offset) {
        if (getYear(date) != getYear(new Date()) && offset == 0) {
            return false;
        }
        Calendar calendar = Calendar.getInstance();
        ...
        Date date1 = DateUtil.getDate(DateUtil.getMonthFirstDay(DateUtil.addMonth(date, -offset)));
        Date date2 = DateUtil.getDate(DateUtil.getMonthFirstDay(new Date()));
        if (DateUtil.whenEQ(date1,date2)) {
            return true;
        }
        return false;
    }
    ...
    public static void main(String[] args) {
      //  Date[] dates = computeMonthFirstAndLastDay(new Date(), 0);
      System.out.println(DateUtil.getDateTime("2019-03-28"));
    }
}
