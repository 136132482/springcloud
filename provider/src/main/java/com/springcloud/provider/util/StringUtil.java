...
/**
 * 有关字符串处理的工具类。 这个类中的每个方法都可以“安全”地处理<code>null</code>，而不会抛出
...
public class StringUtil extends StringUtils {
    ...
    public static String strFormat(String str, String tips) {
        return isEmpty(str) ? tips : str;
    }
    ...
    public static boolean isEmpty(Object obj) {
        try {
            if (obj == null) {
                return true;
            } else if ("".equals(obj) || "null".equals(obj) || "".equals(obj.toString())) {
                return true;
            }
        } catch (Exception e) {
            return true;
     ...
     * @return
     */
    public static Integer parseInteger(String arg, Integer defaultValue) {
        Integer value = 0;
        try {
     ...
     * @return
     */
    public static String toURLPath(String url, String charset) {
        if (isEmpty(url)) {
            return "";
        }
        try {
     ...
     * @return
     */
    public static String toURLPath(String url) {
        if (isEmpty(url)) {
            return "";
        }
        try {
     ...
     * 将中文转换为英文
     *
     * @param chineseStr 汉子串
     * @return
     */
    public static String getEnglishByChinese(String chineseStr) {
        HanyuPinyinOutputFormat pyFormat = new HanyuPinyinOutputFormat();
        pyFormat.setCaseType(HanyuPinyinCaseType.LOWERCASE);
        pyFormat.setToneType(HanyuPinyinToneType.WITHOUT_TONE);
        pyFormat.setVCharType(HanyuPinyinVCharType.WITH_V);
        try {
            return PinyinHelper.toHanyuPinyinString(chineseStr, pyFormat, "");
    ...
    /**
     * 功能描述: 获取汉字串拼音首字母，英文字符不变
     *
     * @param chineseStr 汉字串
     * @return 汉语拼音首字母
     */
     ...
     * @return 汉语拼音
     */
    public static String getFullSpell(String chineseStr) {
        StringBuffer pybf = new StringBuffer();
        char[] arr = chineseStr.toCharArray();
        HanyuPinyinOutputFormat defaultFormat = new HanyuPinyinOutputFormat();
        defaultFormat.setCaseType(HanyuPinyinCaseType.LOWERCASE);
    ...
    public static String getUpEname(String name) {
        ...
        // 名字的长度
        if (strs.length == 2) {
            newname = toUpCase(getEnglishByChinese("" + strs[0])) + " " + toUpCase(getEnglishByChinese("" + strs[1]));
        } else if (strs.length == 3) {
            newname = toUpCase(getEnglishByChinese("" + strs[0])) + " " + toUpCase(getEnglishByChinese("" + strs[1] + strs[2]));
        } else if (strs.length == 4) {
            newname = toUpCase(getEnglishByChinese("" + strs[0] + strs[1])) + " " + toUpCase(getEnglishByChinese("" + strs[2] + strs[3]));
        } else {
            newname = toUpCase(getEnglishByChinese(name));
        }
     ...
     * 自定义的字符拼接
     *
     * @param arg1          要操作的str null自动转为"";
     * @param arg2          要拼接的str
     * @param arg3          拼接符
     * @param isAppendAfter 是否在最后拼接
     * @param isAddArg1     是否先补充arg1的前后拼接符
     * @return 拼接之后的字符
     * @author dwk 2015年12月14日下午8:48:57
     */
    public static String appendStr(String arg1, String arg2, String arg3, boolean isAppendAfter, boolean isAddArg1) {
        // 1.拼接目标字符串 split+splitStr+split 判断是否存在
        // 2.判断之前之后拼接
        if (null == arg1) {
            arg1 = "";
        }
        ...
        if (isEmpty(arg1)) {
            ...
            if (isAddArg1) {
                // 需要补全
                if (!arg1.startsWith(arg3)) {
                    sb.insert(0, arg3);
                }
                if (!arg1.endsWith(arg3)) {
                    sb.append(arg3);
                }
            }
        }
        ...
        if (!isAppendAfter) {
            // ,1
            splitsb.insert(0, arg2).insert(0, arg3);
            // ,1 ,
            checksb.append(splitsb).append(arg3);

        } else {
            // 1,
            splitsb.append(arg2).append(arg3);
            // , 1,
            checksb.insert(0, arg3).append(splitsb);
        }
        ...
        if (sb.indexOf(checksb.toString()) == -1) {
            // 不存在
            if (!isAppendAfter) {
                sb.insert(0, splitsb);
            } else {
     ...
     *
     * @param arg1          要操作的str
     * @param arg2          要删除的拼接str
     * @param arg3          要删除的拼接符
     * @param isAddArg3     是否拼接arg3
     * @param isAppendAfter 是否在最后拼接
     * @return 拼接之后的字符
     * @author dwk 2015年12月14日下午8:48:57
     */
    public static String delAppendStr(String arg1, String arg2, String arg3, boolean isAddArg3, boolean isAppendAfter) {
        StringBuffer delStr = new StringBuffer(arg2);
        if (isAddArg3) {
            if (!isAppendAfter) {
                delStr.insert(0, arg3);
            } else {
                delStr.append(arg3);
            }
        }

        return arg1.replace(delStr, "");
    }
     ...
     *
     * @param str       字符源
     * @param maxLength 字符串的长度不会超过Integer.MAX_VALUE
     * @return
     * @author dwk 2015年1月27日下午4:29:42
     */
     ...
     * 拼接mysql in 语句
     */
    public static String parseMysqlIn(List<String> list) {
        String value = "";
     ...
     * 相等匹配
     * <p>
     * 从arg3里对比是否有与arg1相等的 有返回相等元素，没有，返回arg2默认值
     * 如果arg1==null&&arg3==null&&arg3.length==0 返回arg2默认值 传入时要保证 arg1 arg3 类型一致
     *
     * @param arg1 审核对象
     * @param arg2 默认值
     * @param arg3 审核数组 不会核对里面是否有重复值，没有必要
     * @return
     * @author dwk 2015年2月27日上午10:46:30
     */
    public static String equalMatching(String arg1, String arg2, String... arg3) {
        try {
            if (null != arg1 && null != arg3 && arg3.length > 0) {
                for (String obj : arg3) {
                    if (arg1.equals(obj)) {
                        return obj;
                    }
    ...
    /**
     * 字符串匹配正则表达式 null 返回false
     *
     * @param regex 正则
     * @param str   比较字符串
     * @return true 正确匹配
     * @author dwk 2015年1月13日下午2:12:57
     */
        ...
        if (text != null && !text.isEmpty() && searchList != null && searchList.length != 0 && replacementList != null && replacementList.length != 0) {
            if (timeToLive < 0) {
                ...
                if (searchLength != replacementLength) {
                    throw new IllegalArgumentException("Search andReplace array lengths don\'t match: " + searchLength + " vs " + replacementLength);
                } else {
    ...
    /**
     * 将实体字段转换成符合自己数据库风格的字段
     *
     * @param value
     * @return
     */
    public static String fieldToSql(String value) {
        if (StringUtil.isEmpty(value)) {
            return value;
        }
        return toLowerCaseWithUnderscores(value) + "_";
    }

    public static void main(String[] args) {
        System.out.println(StringUtil.getToken().replace("-", ""));
