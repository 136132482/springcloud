package com.springcloud.provider.excel;
...
public class ExcelUtils {
    ...
    public void exportObjects2Excel(JSONArray data, Class clazz, List<String> fields, boolean isWriteHeader, String sheetName, String targetPath)
            throws Exception {
        FileOutputStream fos = new FileOutputStream(targetPath);
        exportExcelNoModuleHandler(data, clazz, fields, isWriteHeader, sheetName, true).write(fos);
    }
    ...
    private Workbook exportExcelNoModuleHandler(JSONArray data, Class clazz, List<String> fields, boolean isWriteHeader, String sheetName,
                                                boolean isXSSF) throws Exception {
        ...
        if (isWriteHeader) {
            // 写标题
            for (int i = 0; i < headers.size(); i++, index++) {
                if (fields != null && fields.size() > 0 && fields.contains(headers.get(i).getFiled())) {
                    row.createCell(index).setCellValue(headers.get(i).getTitle());
        ...
        for (int i = 0; i < data.size(); i++) {
            row = sheet.createRow(i + 1);
            _data = data.getJSONObject(i);
            index = 0;
            for (int j = 0; j < headers.size(); j++, index++) {
                if (fields == null || fields.size() == 0) {
                    if (headers.get(j).getFiled().contains(".")) {
                        String[] array = headers.get(j).getFiled().split("\\.");
                        row.createCell(j).setCellValue(_data.getJSONObject(array[0]).getString(array[1]));
                    } else {
                        row.createCell(j).setCellValue(_data.getString(headers.get(j).getFiled()));
                    }
                } else if (fields != null && fields.size() > 0 && fields.contains(headers.get(j).getFiled())) {
                    if (headers.get(j).getFiled().contains(".")) {
                        String[] array = headers.get(j).getFiled().split("\\.");
                        row.createCell(index).setCellValue(_data.getJSONObject(array[0]).getString(array[1]));
                    } else {
                        row.createCell(index).setCellValue(_data.getString(headers.get(j).getFiled()));
                    }
                } else {
                    index--;
                }
    ...
    public static void main(String[] args) {
        ...
        try {
            List<Demo> demos = excelUtils.readExcel2Objects("/Users/mac/Documents/用户自定义表格.xlsx", Demo.class,0);
            System.out.println(demos);
//
//            File file = new File("/Users/mac/Documents/用户自定义表格.xlsx");
//            InputStream is = new FileInputStream(file);
//            List<List<String>> lists = excelUtils.readExcel2List(is);
//            System.out.println(lists);

//            List<Demo>list =new ArrayList<>();
//            Demo demo = new Demo();
//            demo.setName("21312");
//            demo.setPhone("1231212");
//            demo.setUser("2312323");
//            Map map = new HashMap();
//            map.put("phone_","1111");
//            map.put("user_","2222");
//            map.put("name_","3333");
//            excelUtils.exportObjects2Excel("/Users/mac/Documents/用户自定义表格.xlsx",demos,Demo.class,"/Users/mac/Documents/xxx.xlsx");

//             excelUtils.exportObjects2Excel(demos,Demo.class,true,"/Users/mac/Documents/xxx.xlsx");
             List<List<String>>list = new ArrayList<>();
            ...
            list.add(list2);
             ...
             list1.add("用户");
             list1.add("电话");
             excelUtils.exportObjects2Excel(list,list3,"/Users/mac/Documents/zzz.xlsx");
        } catch (Exception e) {
            e.printStackTrace();
        }
