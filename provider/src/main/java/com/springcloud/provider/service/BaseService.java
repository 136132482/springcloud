...
import com.springcloud.provider.persistence.BaseDao;
...
public abstract class BaseService<D extends BaseDao<T>, T extends DataEntity>  {
    ...
    public Integer insert(T entity) {
        ...
        if (isOk > 0) {
            //保存到缓存
            CacheManagerFactory.getInstance().set(entity.getId(), getById(entity.getId()));
        }
        return isOk;
    }
    ...
    public Integer insertList(List<T> entityList) {
        ...
        if (isOk > 0) {
            //保存到缓存
            for (T t : entityList) {
                CacheManagerFactory.getInstance().set(t.getId(), getById(t.getId()));
            }
        }
        return isOk;
    }
...
//        }
//        //优先读取缓存
//        T t = CacheManagerFactory.getInstance().get((String) id);
