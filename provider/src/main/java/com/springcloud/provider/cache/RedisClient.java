...
import com.springcloud.provider.util.ObjectUtils;
import com.springcloud.provider.util.SpringContextHolder;
...
//                List<Object> list = jedis.lrange(key, 0, -1);
    ...
    @Override
    public <T> T getWithoutToObject(String key) {
        if (StringUtil.isBlank(key)) {
            return null;
        }
        T result = null;
